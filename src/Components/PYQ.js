  ...
  const handleInputChange = useCallback(
    ...
    debounce(() => {
      ...
      if (input.length) {
          ...
          axios.post("https://ng-frontend-1.onrender.com/api/GetPhysicsCycleSubjects", searchTerm)
          .then(response1 => {
            const physicsCycleData = response1.data;

            // Second API call for Chemistry Cycle
            setTimeout(() => {
              axios.post("https://ng-frontend-1.onrender.com/api/GetChemistryCycleSubjects", searchTerm)
                .then(response2 => {
