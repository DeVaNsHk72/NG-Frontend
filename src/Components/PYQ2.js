  ...
  const ShowSelectedCycleRelatedPdf = ()=>{
    ...
    if(Sem3){
      ...
      const myData = {Branch:Branch,Sem:3}
        ...
        axios.post("https://ng-backend-1-f4r9.onrender.com/api/getBranchRelatedPYQ",myData)
        .then(response=>{
    ...
    debounce(() => {
      ...
      if (input.length) {
          ...
          searchTerm = { Branch:Branch,Sem:Sem };
            ...
            setTimeout(() => {
              axios.post("https://ng-backend-1-f4r9.onrender.com/api/getBranchRelatedPYQ", searchTerm)
                .then(response2 => {
