...
const API_ENDPOINTS = {
  BRANCH_PYQ: `${API_BASE_URL}/getBranchRelatedPYQ`,
  SEARCH_BRANCH_PYQ: `${API_BASE_URL}/getSearchedBranchRelatedPYQ`, // Assuming this endpoint exists for search
};
...
const fetchPYQsByFilter = async (filters) => {
  ...
  const body = { Branch: branch, Sem: sem };
  const data = await postRequest(API_ENDPOINTS.BRANCH_PYQ, body);
  return uniqueSubjects(data);
};

const searchPYQs = async (query, branch) => {
  if (!query || !branch) return [];
  const body = { SubjectName: query, Branch: branch };
  // Using an assumed search endpoint for better functionality
  const data = await postRequest(API_ENDPOINTS.SEARCH_BRANCH_PYQ, body).catch(() => {
    // Fallback to filtering the main list if search endpoint fails
    return fetchPYQsByFilter({branch, sem: 3}).then(res => res.concat(fetchPYQsByFilter({branch, sem: 4})))
  });
  const allResults = uniqueSubjects(data);
  return allResults.filter(s => s.SubjectName.toLowerCase().includes(query.toLowerCase()));
};
    ...
    <>
        <aside className={`fixed top-0 left-0 z-50 w-64 h-full bg-slate-900 text-white flex flex-col gap-8 py-6 px-4 transition-transform duration-300 ease-in-out md:fixed md:top-16 md:h-[calc(100vh-4rem)] md:translate-x-0 overflow-y-auto ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div className="flex flex-col gap-3">
                <h3 className="text-blue-400 text-lg font-semibold px-2">Branch</h3>
                {BRANCH_OPTIONS.map(({ id, label }) => <SidebarButton key={id} onClick={() => onFilterChange('branch', id)} isActive={filters.branch === id}>{label}</SidebarButton>)}
            </div>
            <div className="flex flex-col gap-3">
                <h3 className="text-blue-400 text-lg font-semibold px-2">Semester</h3>
                {SEM_OPTIONS.map(({ id, label }) => <SidebarButton key={id} onClick={() => onFilterChange('sem', id)} isActive={filters.sem === id} disabled={!filters.branch}>{label}</SidebarButton>)}
            </div>
        </aside>
...
const PYQRow = ({ subject }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-center bg-slate-800 p-4 rounded-lg border border-slate-700">
...
// --- MAIN COMPONENT ---
export default function SecondYearPYQ() {
    ...
    const handleFetchData = useCallback(async () => {
        if (debouncedSearchTerm) {
            ...
            if (!filters.branch) {
                ...
                setPyqs([]); setLoading(false);
                return;
            }
            try {
                const results = await searchPYQs(debouncedSearchTerm, filters.branch);
                setPyqs(results);
    ...
    const handleFilterChange = (key, value) => {
        ...
        setFilters(prev => {
            ...
            if (key === 'branch') newFilters.sem = null;
            return newFilters;
        });
    };
    
    const showInitialMessage = !loading && !error && pyqs.length === 0 && !(filters.branch && filters.sem) && !debouncedSearchTerm;
                    ...
                    <div className="w-full max-w-4xl mx-auto flex flex-col gap-6">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full h-12 pl-12 pr-4 bg-slate-800 border border-slate-700 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none transition disabled:opacity-50 disabled:cursor-not-allowed"
                            />
                        </div>
                        <div className="min-h-[calc(100vh-20rem)] flex flex-col">
                            ...
                            {showInitialMessage && (
                                <div className="text-center p-8 bg-slate-800/50 rounded-lg animate-fade-in m-auto">
                                    ...
                                    <p className="text-slate-400 mt-2">Choose a Branch and Semester to find PYQs.</p>
                                </div>
                            )}
