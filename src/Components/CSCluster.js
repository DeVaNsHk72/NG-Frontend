  ...
  const ShowSelectedCycleRelatedPdf = ()=>{
    ...
    if((Sem1 || Sem2) && PhysicsCycle){
      ...
      setSelectedSubjectNumber([])
        ...
        axios.post("https://ng-backend-y7a6.onrender.com/api/PhysicsCycle/GetAllModules",myData)
        .then(response=>{
    ...
    if((Sem1 || Sem2) && ChemistryCycle){
      ...
      setShowSubjectsClicked(true)
        ...
        var myData = {Category:"CS",Sem:Sem}
        axios.post("https://ng-backend-y7a6.onrender.com/api/ChemistryCycle/GetAllModules",myData)
        .then(response=>{
    ...
    debounce(() => {
      ...
      if (input.length) {
        ...
        if (input.trim() === '') {
          ...
          axios.post("https://ng-backend-y7a6.onrender.com/api/GetPhysicsCycleSubjects", searchTerm)
          .then(response1 => {
            const physicsCycleData = response1.data;

            // Second API call for Chemistry Cycle
            setTimeout(() => {
              axios.post("https://ng-backend-y7a6.onrender.com/api/GetChemistryCycleSubjects", searchTerm)
                .then(response2 => {
...
<div className="flex flex-row gap-2 bg-custom-dark border-2 border-custom-dark mt-2 rounded-lg shadow-lg p-4 mx-auto w-full max-w-3xl">
...
<div className="flex items-center justify-between border-2 border-custom-dark rounded-xl p-2 md:p-4 w-full">
  ...
  {pdf.Syllabus !== "" ? (
    ...
    <div className={`grid grid-cols-4 gap-2 bg-[#545454] border-2 border-custom-dark bg-custom-dark rounded-xl shadow-lg p-2 justify-between w-full transition-opacity duration-700 ease-in-out `}>
